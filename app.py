...
# Direct OpenCV capture, exactly like webcam.py
...
def detect_emotion(frame):
    ...
    for (x, y, w, h) in faces:
        ...
        transformed_tensor = transform(face_rgb).unsqueeze(0)
    ...
    # Gradio's gr.Image display expects RGB, so convert before returning
...
with gr.Blocks(theme=theme, css=custom_css, title="EmotionLens") as demo:
    ...
    output_image = gr.Image(label=None, show_label=False)
    timer = gr.Timer(0.1)
    timer.tick(fn=get_frame, inputs=None, outputs=output_image)
